...
def bin_loader(path: str):
    image = np.load(path.replace('.jpg','.npy').replace('.bmp','.npy'))
    image = torch.from_numpy(image/255).type(torch.float32)
    return image

def val_model(model,cfg,imgbin=False, train_val=False):
    ...
    # padding = (32,32,32,32,1,1)
    padding = (0,0,0,0,1,1)
    if train_val:
        datas_gallery=eval(cfg.datasets.name)(cfg.val.root,cfg.datasets.gallery,label_dict,istrain=False)
    else:
        if imgbin:
            datas_gallery=eval(cfg.datasets.name)(cfg.val.root,cfg.val.gallery,label_dict,loader=bin_loader,istrain=False)
        else:
            datas_gallery=eval(cfg.datasets.name)(cfg.val.root,cfg.val.gallery,label_dict,istrain=False)
    ...
    for i, (image, target, _, _) in enumerate(gallery_loader):
        with torch.no_grad():
            ...
            if imgbin:
                server_img = F.pad(image, padding, mode='constant', value=0).to(local_rank)
                # adv_1dim = torch.cat((image,torch.zeros(image.size(0), 1, 64, 64).to(local_rank)),dim=3)
                # adv_2dim = torch.cat((image,torch.zeros(image.size(0), 1, 64, 128).to(local_rank)),dim=2)
                # server_img=torch.cat((adv_2dim, torch.zeros(image.size(0),2 , 128, 128).to(local_rank)), dim=1)
                # server_img = image
            else:
    ...
    if train_val:
        datas_probe=eval(cfg.datasets.name)(cfg.val.root,cfg.val.probe,label_dict,istrain=False)
    else:
        if imgbin:
            datas_probe=eval(cfg.datasets.name)(cfg.val.root,cfg.val.probe,label_dict,loader=bin_loader,istrain=False)
        else:
            datas_probe=eval(cfg.datasets.name)(cfg.val.root,cfg.val.probe,label_dict,istrain=False)
    ...
    with torch.no_grad():
        for i, (image, target, _, _) in enumerate(probe_loader):
                ...
                if imgbin:
                    ...
                    server_img = _dct_to_images(server_inputs,cfg.train.sub_channels)
                    # server_img = image
                # server_inputs = F.normalize(server_inputs,dim=0)
    ...
    if local_rank == 0 or -1:
        ...
        # logger.info(f'Rank1 accuracy is {avg_r_a} + {std_r_a}')
        # logger.info(f'VR@FAR=0.1% accuracy is {avg_v_a} + {std_v_a}')
...
def compute_metric(score, probe_pid, gallery_pid):
    ...
    # logger.info(f"AUC {auc_score:.2%}, EER {eer:.2%}")
...
if __name__=="__main__":
    local_rank = int(os.environ["LOCAL_RANK"])
    cfg=get_config('config/NIR-VISv2.yml')
    
    dist.init_process_group(backend='nccl')
    torch.cuda.set_device(local_rank)
    # multiprocessing.set_start_method('spawn')
    model=LightCNN_V4(num_classes=cfg.datasets.num_classes)
    ...
    try:
        val_model(model,cfg,imgbin=True)
