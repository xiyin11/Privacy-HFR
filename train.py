...
def train(model,datas_train,cfg): 
    ...
    for epoch in range(0,cfg.train.epochs):
        ...
        if epoch%cfg.train.val_frep==0:
            start = time.perf_counter()
            stat = val_model(model,cfg,train_val=True)
            if local_rank == 0 or -1:
                if max_rank1 <= stat[0]:
                    max_rank1 = stat[0]
                    best_model_name = cfg.datasets.name + "_" + str(stat[0]*100)[:6] + "" + time.time() + '.ckpt'
...
# OMP_NUM_THREADS=16 torchrun --nproc_per_node 2 train.py